...
UaScope is a web-based OPC UA client for browsing, inspecting and testing OPC UA
...
```text
┌───────────────────────────────────────────────────────────────────────────────┐
│ UaScope  [ opc.tcp://localhost:50000 ] [Connect] [Endpoints…] 👤  ⧉new window │ ← top bar
├───────────────┬────────────────────────────────────┬──────────────────────────┤
...
address box and click **Connect**. UaScope discovers the server's endpoints and
...
Click **Endpoints…** instead of Connect. UaScope lists every endpoint the server
...
1. **You trust the server.** The first time UaScope sees a server's certificate it
   shows a dialog with the subject, issuer, thumbprint and validity dates.
   - **Trust once** — accept for this browser session only.
   - **Trust permanently** — save the certificate to UaScope's trusted store so you
...
2. **The server trusts you.** UaScope identifies itself with its own certificate
   (created automatically on first run, stored under
   `~/.local/share/UaScope/pki` on Linux, `%LOCALAPPDATA%\UaScope\pki` on Windows).
   Most real servers *reject unknown clients* by default: your first secure
   connection attempt fails, and the server puts UaScope's certificate in its
   "rejected" list. Go to the server's own certificate management (its web page,
   engineering tool, or `pki/rejected` folder), mark the UaScope certificate as
...
server restarts, UaScope reconnects automatically and re-establishes its
...
- **Text search**: type part of a name (`pallet`, `temperature`) and press Enter.
  UaScope sweeps the address space breadth-first and lists up to 50 matches; click
...
- **⧉ link** — copy a shareable URL that opens UaScope, connects to this server
...
- Click **Subscribe**. UaScope subscribes to condition events on the Server object
...
- **✔ ack** acknowledges a condition. The comment sent with it is editable in the
  header field (default: "Acknowledged via UaScope").
...
This is where UaScope shows you what is *inside* an `ExtensionObject` that other
...
- **Deep links**: `http://<uascope-host>:5080/?endpoint=opc.tcp://…&node=ns=3;s=…`
  opens UaScope, connects and reveals the node. The **⧉ link** button builds these
  for you. Anyone with network reach to the UaScope host and the server can follow
...
| Certificate trusted, connection still fails with a security error | The *server* has not trusted UaScope's certificate yet. Approve the "UaScope" client certificate in the server's certificate manager (often a `pki/rejected` folder or a web UI), then reconnect. |
...
| **Server / client** | The server exposes data (PLC, gateway, historian); the client consumes it (UaScope, HMI, SCADA, MES). |
...
| **ExtensionObject / UDT** | A structured value (like a PLC user-defined type). UaScope decodes these into readable trees. |
